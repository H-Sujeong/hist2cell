> **Notion 업로드용 패키지** — 이 폴더의 `findings.md` 와 PNG 7장이 같은 디렉토리에 함께 있습니다. Notion 의 File > Import > Markdown & CSV 로 zip 째 업로드하면 이미지가 자동으로 inline 첨부됩니다. 원본은 `inference/analysis_spatial/1_085_12/findings.md` 참고.
>
> ⚠️ epithelial-activity proxy (strict / broad) 해석은 `inference/analysis/EPITHELIAL_PROXY_METHODOLOGY.md` 필독 — 본 score 들은 lung-derived spatial proxy 로서 breast tumor detector 가 아님.
> 80×80 Moran R clustermap 읽는 법은 `inference/analysis/MORAN_R_METHODOLOGY.md` §3.4 참조.
