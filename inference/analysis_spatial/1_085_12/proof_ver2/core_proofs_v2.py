"""slide1 (1_085_12) — proof_ver2: data-driven cross-modality validation.
...
from _proof_ver2_lib import (
    ...
    discover_marker_pairs, per_roi_cosine, roi_cosine_similarity,
)
...
CFG = SlideConfig(
    ...
    roi_pkl=HERE.parent / "1_085_12_ROI_groups.pkl",
    npy=HERE.parent / "meteo_1_085_12_coords.npy",
...
def plot_cca_scatter(Hc, Pc, sections, train_rs, out_path):
    ...
    fig.suptitle(f"CCA — slide1: data-driven canonical correlations between modalities",
                 fontsize=12)
    fig.tight_layout()
...
def plot_permutation_null(null_top, observed, out_path):
    ...
    ax.set_title(f"slide1 — permutation null for top canonical r  "
...
def plot_top_loaders(h_load, p_load, cell_cols, gene_index, axis_idx,
    ...
    fig.suptitle(f"CCA loadings — slide1 canonical axis {axis_idx+1}", fontsize=12)
    fig.tight_layout()
    fig.savefig(out_path, dpi=130, bbox_inches="tight")
    plt.close(fig)
...
def plot_per_roi_similarity(sims, sections, out_path):
    ...
    order = np.argsort([SECTION_ORDER.index(s) for s in sections] +
    ...
    ax.set_title("slide1 — per-ROI cross-modality similarity "
...
def main():
    ...
    # also a small top-pair heatmap: row = top 20 positive pairs, two cols = r value
    ...
    if len(top20) > 0:
        ...
        ax.invert_yaxis()
        ax.set_title("slide1 — top 20 data-driven positive marker-celltype pairs (BH<0.05)",
                     fontsize=11)
        fig.tight_layout()
