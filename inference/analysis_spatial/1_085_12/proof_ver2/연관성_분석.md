...
- ratio = intra / inter < 1 이면 section 내가 더 가까움 (section 구조 존재)
- ratio ≈ 1 또는 > 1 이면 section 라벨이 ROI 들을 묶지 못한다는 신호
- 1000회 ROI section 라벨 shuffle 로 영가설 95% 구간을 만들고 비교.

![axis intra/inter slide1](cca_axis_intra_inter.png)

**숫자 표**

| axis | modality | intra mean | inter mean | ratio | null 95% 구간 | p (perm) | 판정 |
|---|---|---|---|---|---|---|---|
| 1 | Hist2Cell | 0.50 | 1.33 | **0.376** | [0.798, 1.144] | <0.001 | **매우 강함** |
| 1 | Proteomics | 0.46 | 1.36 | **0.339** | [0.806, 1.137] | <0.001 | **매우 강함** |
| 2 | Hist2Cell | 0.72 | 1.26 | **0.571** | [0.798, 1.175] | <0.001 | 강함 |
| 2 | Proteomics | 0.69 | 1.27 | **0.546** | [0.807, 1.169] | <0.001 | 강함 |
| 3 | Hist2Cell | 1.05 | 1.17 | 0.897 | [0.801, 1.169] | 0.137 | **null floor 안 → 약함** |
| 3 | Proteomics | 1.08 | 1.12 | 0.963 | [0.779, 1.183] | 0.351 | **null floor 안 → 약함** |

**핵심 메시지.**
- **axis 1**: ratio 0.34–0.38 (관측치가 null 95% 하한 0.80 보다 한참 작음)
  → section 내가 section 간보다 *두 배 이상* 가깝다. 두 modality
  모두에서 매우 강한 section 구조.
- **axis 2**: ratio 0.55–0.57, null 분포 바깥에 명확히 떨어져 있음 (p<0.001).
  axis 1 보다는 약하지만 **a vs b 의 분리는 통계적으로도 진짜**.
- **axis 3**: ratio 0.90–0.96, **관측치가 null 95% 구간 [0.80, 1.18] 안에
  완전히 들어가 있음** (p=0.14 / 0.35). → "Tumor 내부의 상피 분화 결" 이라는
  axis 3 해석은 **시각적 분리에 불과하고 통계적으로는 우연과 구분되지
  않는다**. axis 3 단독으로 어떤 주장도 만들지 말 것.

### Axis 1/2/3 종합 보고 문장

> "두 modality 의 CCA canonical axis 1 은 *상피·선조직 ↔ 면역·혈관·기질*
> 의 main gradient 를 *통계적으로 매우 강하게* 잡았고 (intra/inter
> ratio = 0.34–0.38, p<0.001; Tumor 계열은 axis 1 +, T-cell 계열은
> axis 1 −), axis 2 는 *Tumor 내부 이질성* (a vs b) 을 추가로 분리하며
> 이 역시 통계적으로 유의 (ratio = 0.55–0.57, p<0.001). 다만 **axis 3
> 는 시각적 분리는 있어도 permutation null 안에 들어가 통계적 의미가
> 없음** (p=0.14 / 0.35) — axis 3 의 'High-risk 는 alveolar/basal stress
> keratin 쪽 / Low-risk 는 plasma-B/mucous 쪽' 해석은 보조 관찰 수준
> 으로만 사용하고 단독 주장은 만들지 않는다."
