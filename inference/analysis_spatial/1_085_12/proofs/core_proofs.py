...
def plot_roi_top_heatmap(sig, groups, top_df, out_path):
    ...
    # Separate axes for the section color strip (left) and the heatmap
    # so the strip doesn't bleed into the first data column and the
    # legend / colorbar don't crowd the last column.
    ...
    # colorbar in its own slim axes
    cbar = fig.colorbar(im, cax=ax_cbar)
    cbar.set_label("z (across ROIs)", fontsize=8)
    cbar.ax.tick_params(labelsize=7)

    # Section legend OUTSIDE the right colorbar (above)
    ...
    fig.suptitle("Per-ROI top cell-type heatmap "
