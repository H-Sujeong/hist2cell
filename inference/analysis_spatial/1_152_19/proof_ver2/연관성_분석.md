...
### 각 axis 의 section 분리가 *통계적으로도* 의미있는가 — intra/inter check

위 dumbbell·section means 의 *시각적* 분리가 진짜 신호인지를 axis 별로
정량 검정. 각 ROI 의 canonical score 만 가지고, **section 내 ROI 쌍의
|Δ score| 평균** 과 **section 간 ROI 쌍의 |Δ score| 평균** 을 비교한다.

![axis intra/inter slide2](cca_axis_intra_inter.png)

**숫자 표**

| axis | modality | intra mean | inter mean | ratio | null 95% 구간 | p (perm) | 판정 |
|---|---|---|---|---|---|---|---|
| 1 | Hist2Cell | 0.49 | 1.11 | **0.437** | [0.791, 1.150] | <0.001 | **매우 강함** |
| 1 | Proteomics | 0.47 | 1.12 | **0.421** | [0.789, 1.149] | <0.001 | **매우 강함** |
| 2 | Hist2Cell | 1.11 | 1.17 | 0.951 | [0.876, 1.098] | 0.188 | **null floor 안 → 약함** |
| 2 | Proteomics | 1.10 | 1.16 | 0.945 | [0.877, 1.099] | 0.164 | **null floor 안 → 약함** |
| 3 | Hist2Cell | 1.00 | 1.15 | 0.873 | [0.854, 1.113] | 0.041 | **보더라인** |
| 3 | Proteomics | 1.05 | 1.16 | 0.911 | [0.866, 1.109] | 0.086 | **보더라인** |

**핵심 메시지.**
- **axis 1**: ratio 0.42–0.44 (관측치가 null 95% 하한 0.79 보다 한참 작음)
  → section 내가 section 간보다 *두 배 이상* 가깝다. 두 modality 모두에서
  매우 강한 section 구조. slide1 axis 1 (ratio ~0.36) 과 거의 동일한 강도.
- **axis 2**: ratio 0.95, **관측치가 null 95% 구간 [0.88, 1.10] 안에 완전히
  들어감** (p=0.17–0.19). → "v (ctrl) 가 axis 2 + 극단" 이라는 section means
  의 분리는 **n=5 짜리 작은 group 의 평균이 운으로 튄 시각적 효과** 일
  가능성이 크다. 통계적으로 v vs e 분리를 주장하지 말 것.
- **axis 3**: ratio 0.87–0.91, null 95% 구간의 *경계 근처* (Hist2Cell p=0.04,
  Proteomics p=0.09). → "h (Low-risk T-cell) 가 stromal/ECM 풍부" 라는
  매력적인 발견이 **Hist2Cell 측에서는 보더라인 유의, Proteomics 측에서는
  유의하지 않음**. 추가 데이터 (다른 슬라이드, 더 많은 ROI) 없이 단독
  주장으로 만들기에는 약함.

**slide1 과의 비교.** slide1 은 axis 2 가 강하게 유의 (p<0.001) 했지만,
slide2 는 axis 2 도 axis 3 도 유의하지 않다. 즉 **slide2 는 사실상 axis 1
하나만 통계적으로 section 구조를 잡는다**. slide2 가 slide1 보다 *section
구조가 더 적은 차원에 압축* 되어 있음을 의미.

### Axis 1/2/3 종합 보고 문장

> "두 modality 의 CCA canonical axis 1 은 *선상피·분비·대사 ↔ 혈관·
> 평활근·혈액* 의 main gradient 를 *통계적으로 매우 강하게* 잡았고
> (intra/inter ratio = 0.42–0.44, p<0.001; Tumor 계열은 axis 1 −,
> T-cell 계열은 axis 1 +; 특히 g 가 +1.9 로 압도적). **다만 axis 2 와
> axis 3 는 permutation null 검정에서 section 분리가 통계적으로
> 유의하지 않거나 보더라인** (axis 2 p=0.17/0.19, axis 3 p=0.04/0.09)
> 이라, 그림에서 보이는 *v 의 axis 2 + 극단* 이나 *h 의 axis 3 − 극단*
> 같은 분리는 **작은 group 의 평균 fluctuation** 으로 설명될 수 있다.
> 따라서 slide2 의 cross-modality 일치 주장은 axis 1 단독에 한정한다."
...
| `cca_axis_intra_inter.png` / `.csv` | axis 별 intra/inter ratio + 1000-perm null |
